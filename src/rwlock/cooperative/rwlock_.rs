...
/// # 公平性
///
/// 普通读者在没有写者持有、且队列中没有写者/升级等待者时走无锁快速路径；
/// 队列里只有读者时，新读者仍然可以插队。写者与可升级读者则需要队列为空
/// 才能走快速路径；写者/升级一旦入队就会挡住后续读者的快速路径，因此不会被
/// 持续到达的读者饿死。
///
/// # 读者计数上界
///
/// 状态字把 `D` 的最高四位用作标志位，读者计数占其余位，因此读者上限是
/// `D::MAX >> 4`。默认 `usize` 下为 `2^60 - 1`；若显式使用较窄的 `D`
/// （例如 `u8`），上限会显著下降（`u8` 下为 15）。
///
/// # 关于移动
///
